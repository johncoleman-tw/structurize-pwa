structurizr.ui.Diagram = function(id, diagramIsEditable, constructionCompleteCallback) {
    ...
    this.reset = function() {
        ...
        // Clear the cached current view so that a subsequent changeView(sameKey)
        // is not treated as a no-op.  This is essential after the workspace JSON
        // is replaced (e.g. on a source-editor save/reparse): changeView skips
        // rendering when view.key === currentView.key, even though the underlying
        // workspace data has changed.  Clearing here forces changeView to look up
        // the view from the new structurizr.workspace and re-render.
